...
$(function(){
  ...
  if (qOpen === 'add') {
    cargarLocalidades().done(function(){
      if (qLoc) { $('#id_localidad').val(qLoc); }
      cargarSedes(qLoc).done(function(){ if (qSede) { $('#id_sede').val(String(qSede)); } new bootstrap.Modal(document.getElementById('modalPlano')).show(); });
    });
  }
});
</script>
