        ...
        if ($modoRemito === 'nuevo') {
            ...
            for ($i = 0; $i < $maxRetries; $i++) {
                ...
                try {
                    ...
                    if (strpos($lastErr, '1062') === false) {
                        throw $e;
                    }
                    // Sincronizar secuencia si hay conflicto
                    try {
                        $anioNow = (int) date('Y');
                        $m = $conexion->prepare("SELECT MAX(CAST(SUBSTRING_INDEX(numero_remito, '_', 1) AS UNSIGNED)) AS maxseq FROM remitos WHERE RIGHT(numero_remito, 4) = ?");
                        $m->execute([strval($anioNow)]);
                        $max = (int) ($m->fetch()['maxseq'] ?? 0);
                        $conexion->prepare("UPDATE remito_secuencia SET ultimo = GREATEST(ultimo, ?) WHERE anio = ?")->execute([$max, $anioNow]);
            ...
            if (!$ok) {
                throw new Exception('No se pudo asignar número de remito único: ' . $lastErr);
            }
            $idRemito = (int) $conexion->lastInsertId();

        } else {
            // Modo HISTÓRICO (por defecto): Secuencial usando tabla remitos_historicos_secuencia
            $anio = date('Y');
            $sufijo = '_' . $anio . '_hist';
...
// Determinar modo para mostrar en la interfaz
$modoActual = isset($_GET['modo']) && $_GET['modo'] === 'nuevo' ? 'nuevo' : 'historico';
$tituloModo = $modoActual === 'nuevo' ? 'Insumo Nuevo' : 'Insumo Histórico';
$badgeModo = $modoActual === 'nuevo'
    ? '<span class="badge bg-primary ms-2">Remito Actual</span>'
    : '<span class="badge bg-secondary ms-2">Remito Histórico</span>';
?>

<div class="row">
    <div class="col-12">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h4 class="mb-0"><i class="fas fa-plus me-2"></i>Agregar <?php echo $tituloModo; ?>
                Asignado<?php echo $badgeModo; ?></h4>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="mb-2">
                                    ...
                                    <input type="date" class="form-control" id="fecha_asignacion"
                                        name="fecha_asignacion" value="<?php echo date('Y-m-d'); ?>" required>
                                </div>
                            </div>
                            <div class="col-md-6">
                ...
                <div class="row">
                    <div class="col-12">
                        ...
                        <div class="row g-2" id="formulario-campos" style="display: none;">
                            ...
                            <div class="col-md-4">
                                <div class="card h-100 border-0 shadow-sm">
                                    <div class="card-header bg-light py-2">
                                        <h6 class="mb-0"><i class="fas fa-info-circle me-2 text-primary"></i>Información
                                            Básica</h6>
                                    </div>
                                    <div class="card-body p-3">
                                        <div class="mb-2">
                                            <label id="label-nombre-insumo" class="form-label">Nombre del Insumo
                                                *</label>
                                            <input type="text" class="form-control form-control-sm w-100"
                                                id="nombre_insumo" name="nombre_insumo" required>
                                            <small id="help-nombre-insumo" class="form-text text-muted"
                                                style="display: none;">Insumo + Marca + Modelo + Conexión</small>
                                            <div id="invalid-nombre-insumo" class="invalid-feedback">El nombre del
                                                insumo es obligatorio</div>
                                        </div>
                                        ...
                                        <div id="campos-varios" style="display: none;">
                                            <div class="mb-2">
                                                ...
                                                <select class="form-select form-select-sm w-100"
                                                    id="subcategoria_varios" name="subcategoria_varios">
                                            ...
                                            <div class="mb-2">
                                                <label for="descripcion_general" class="form-label">Descripción
                                                    General</label>
                                                <textarea class="form-control form-control-sm w-100"
                                                    id="descripcion_general" name="descripcion_general"
                                                    rows="2"></textarea>
                                            </div>
                                        </div>
                                        ...
                                        <div id="campos-especificos" style="display: none;">
                                            ...
                                            <div class="mb-2">
                                                ...
                                                <input type="text" class="form-control form-control-sm w-100"
                                                    id="id_fisico" name="id_fisico">
                                            </div>

                                            <div class="mb-2">
                                                <label for="id_patrimonio" class="form-label">ID Patrimonio</label>
                                                <input type="text" class="form-control form-control-sm w-100"
                                                    id="id_patrimonio" name="id_patrimonio">
                                            </div>

                                            <div class="mb-2">
                                                <label for="cantidad_especifica" class="form-label">Cantidad</label>
                                                <input type="number" class="form-control form-control-sm w-100"
                                                    id="cantidad_especifica" name="cantidad_especifica" value="1"
                                                    min="1" readonly>
                                                <small class="form-text text-muted">Para este tipo de insumo, la
                                                    cantidad siempre es 1 (carga unitaria)</small>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-4">
                                <div class="card border-0 shadow-sm">
                                    <div class="card-header bg-light py-2">
                                        <h6 class="mb-0"><i class="fas fa-cog me-2 text-primary"></i>Información Común
                                        </h6>
                                    </div>
                                    <div class="card-body px-3 pt-3 pb-0">
                                        <div class="mb-2"><label class="form-label">Fecha de Adquisición</label><input
                                                type="date" class="form-control form-control-sm w-100"
                                                id="fecha_adquisicion_nueva" name="fecha_adquisicion"
                                        ...
                                        <div class="mb-0">
                                            ...
                                            <select class="form-select form-select-sm w-100" id="id_ingreso_nueva"
                                            ...
                                            <small class="text-muted" id="help_ingreso_nueva">Opcional: Asociar a un
                                                ingreso</small>
                                        </div>
                                    </div>
                                </div>
                                <div class="card border-0 shadow-sm mt-2" id="extras-notebook" style="display:none;">
                                    <div class="card-header bg-light py-2">
                                        <h6 class="mb-0"><i class="fas fa-laptop me-2 text-primary"></i>Accesorios
                                            Notebook</h6>
                                    </div>
                                    <div class="card-body p-3">
                                        <div class="form-check form-switch mb-2"><input class="form-check-input"
                                                type="checkbox" id="cargador" name="cargador" value="1"><label
                                                class="form-check-label" for="cargador">Cargador</label></div>
                                        <div class="form-check form-switch mb-2"><input class="form-check-input"
                                                type="checkbox" id="funda" name="funda" value="1"><label
                                                class="form-check-label" for="funda">Funda</label></div>
                                        <div class="row g-2 align-items-center mb-2">
                                            <div class="col-auto">
                                                <div class="form-check form-switch"><input class="form-check-input"
                                                        type="checkbox" id="micro_sd" name="micro_sd" value="1"><label
                                                        class="form-check-label" for="micro_sd">Micro SD</label></div>
                                            </div>
                                            <div class="col"><input type="number" min="0" step="1"
                                                    class="form-control form-control-sm" id="micro_sd_gb"
                                                    name="micro_sd_gb" placeholder="Tamaño (GB)" disabled></div>
                                        </div>
                                        <div class="form-check form-switch mb-2"><input class="form-check-input"
                                                type="checkbox" id="caja" name="caja" value="1"><label
                                                class="form-check-label" for="caja">Caja</label></div>
                                        <div class="form-check form-switch mb-2"><input class="form-check-input"
                                                type="checkbox" id="adaptador_red" name="adaptador_red" value="1"><label
                                                class="form-check-label" for="adaptador_red">Adaptador de red</label>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-4" id="columna-especificaciones">
                                <div class="card h-100 border-0 shadow-sm">
                                    <div class="card-header bg-light py-2">
                                        <h6 class="mb-0"><i
                                                class="fas fa-microchip me-2 text-primary"></i>Especificaciones</h6>
                                    </div>
                                    <div class="card-body p-3">
                                        <div class="campos-especificos" id="campos-pc" style="display: none;">
                                            ...
                                            <div class="mb-2">
                                                ...
                                                <input type="number" class="form-control form-control-sm w-100"
                                                    id="ram_gb" name="ram_gb" min="1" required>
                                        ...
                                        <div class="campos-especificos" id="campos-impresora" style="display: none;">
                                            <div class="mb-2">
                                                ...
                                                <input type="text" class="form-control form-control-sm w-100"
                                                    id="marca_impresora" name="marca_impresora" required>
                                                <div class="invalid-feedback">La marca es obligatoria</div>
                                            </div>
                                            <div class="mb-2">
                                                <label for="modelo_impresora" class="form-label">Modelo *</label>
                                                <input type="text" class="form-control form-control-sm w-100"
                                                    id="modelo_impresora" name="modelo_impresora" required>
                                                <div class="invalid-feedback">El modelo es obligatorio</div>
                                            </div>
                                        </div>
                                        <div class="campos-especificos" id="campos-monitor" style="display: none;">
                                            <div class="mb-2">
                                                ...
                                                <input type="text" class="form-control form-control-sm w-100"
                                                    id="marca_monitor" name="marca_monitor" required>
                                                <div class="invalid-feedback">La marca es obligatoria</div>
                                            </div>
                                            <div class="mb-2">
                                                <label for="modelo_monitor" class="form-label">Modelo *</label>
                                                <input type="text" class="form-control form-control-sm w-100"
                                                    id="modelo_monitor" name="modelo_monitor" required>
                                                <div class="invalid-feedback">El modelo es obligatorio</div>
                                            </div>
                                            <div class="mb-2">
                                                <label for="pulgadas" class="form-label">Pulgadas *</label>
                                                <input type="number" class="form-control form-control-sm w-100"
                                                    id="pulgadas" name="pulgadas" step="0.1" min="1" required>
                                                <div class="invalid-feedback">Las pulgadas son obligatorias</div>
                                            </div>
                                            <div class="mb-2">
                                                <label for="conexion_monitor" class="form-label">Conexión *</label>
                                                <select class="form-select form-select-sm w-100" id="conexion_monitor"
                                                    name="conexion_monitor" required>
                                        ...
                                        <div class="campos-especificos" id="campos-escaner" style="display: none;">
                                            <div class="mb-2">
                                                ...
                                                <input type="text" class="form-control form-control-sm w-100"
                                                    id="marca_escaner" name="marca_escaner" required>
                                                <div class="invalid-feedback">La marca es obligatoria</div>
                                            </div>
                                            <div class="mb-2">
                                                <label for="modelo_escaner" class="form-label">Modelo *</label>
                                                <input type="text" class="form-control form-control-sm w-100"
                                                    id="modelo_escaner" name="modelo_escaner" required>
                                                <div class="invalid-feedback">El modelo es obligatorio</div>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="d-flex justify-content-between mt-3">
                            ...
                            <button type="submit" class="btn btn-primary"><i class="fas fa-save me-1"></i>Guardar y
                                Asignar</button>
                        </div>
                    </div>
                </div>
