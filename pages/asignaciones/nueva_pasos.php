...
<div id="nueva-pasos">
                                    ...
                                    <div class="row g-2 align-items-end">
                                        ...
                                        <div class="col-md-3 d-flex justify-content-end gap-2">
                                            <button type="button" class="btn btn-outline-secondary btn-sm"
                                                id="btnLimpiarFiltros" title="Limpiar filtros"
                                                aria-label="Limpiar filtros"><i class="fas fa-eraser"
                                ...
                                <div class="card-header d-flex justify-content-between align-items-center">
                                    ...
                                    <div class="d-flex align-items-center gap-2">
                                        ...
                                        <button type="button" class="btn btn-outline-primary btn-sm"
                                            onclick="seleccionarFiltrados()" title="Seleccionar filtrados"
                                            aria-label="Seleccionar filtrados"><i class="fas fa-check-double"
                                                aria-hidden="true"></i></button>
                                        <button type="button" class="btn btn-outline-danger btn-sm"
                                            onclick="deseleccionarTodos()" title="Deseleccionar todo"
                                            aria-label="Deseleccionar todo"><i class="fas fa-times"
                                                aria-hidden="true"></i></button>
                                    </div>
                                </div>
                                <div class="card-body">
                                            ...
                                            <tbody>
                                                <?php foreach ($insumos as $ins): ?>
                                                    ...
                                                    <tr class="fila-insumo"
                                                        ...
                                                        data-texto="<?php echo htmlspecialchars($filterText); ?>"
                                                        data-id-fisico="<?php echo htmlspecialchars($ins['id_fisico'] ?? ''); ?>"
                                                        data-numero-serie="<?php echo htmlspecialchars($ins['numero_serie'] ?? ''); ?>">
                                                        <td>
                                                            <div>
                                                                <strong><?php echo htmlspecialchars($displayName); ?></strong>
                                                                <?php
                                                                // Mostrar ID Físico y Número de Serie si existen
                                                                $infoExtra = [];
                                                                if (!empty($ins['id_fisico'])) {
                                                                    $infoExtra[] = 'ID: ' . htmlspecialchars($ins['id_fisico']);
                                                                }
                                                                if (!empty($ins['numero_serie'])) {
                                                                    $infoExtra[] = 'S/N: ' . htmlspecialchars($ins['numero_serie']);
                                                                }
                                                                if (!empty($infoExtra)):
                                                                    ?>
                                                                    <div class="text-muted small">
                                                                        <?php echo implode(' | ', $infoExtra); ?>
                                                                    </div>
                                                                <?php endif; ?>
                                                            </div>
                                                        </td>
                                                        <td>
                                                            <span
                                                                class="badge bg-info"><?php echo htmlspecialchars($tipo); ?></span>
                                                        </td>
                                                        <td><?php echo ($tipo === 'Varios') ? (int) $ins['cantidad'] : 1; ?>
                                                        </td>
                                                        <td><?php echo $ins['punto_stock'] ? htmlspecialchars($ins['punto_stock']) : '<span class="text-muted">Sin punto</span>'; ?>
                                                        </td>
                                                        <td>
                                                            <div
                                                                class="d-flex gap-2 align-items-center justify-content-end flex-wrap">
                                                                <input type="hidden" name="id_insumo[]"
                                                                    value="<?php echo $ins['id_insumo']; ?>"
                                                                    class="hidden-insumo-input"
                                                                    data-tipo="<?php echo htmlspecialchars($tipo); ?>"
                                                                    data-max="<?php echo ($tipo === 'Varios') ? (int) $ins['cantidad'] : 1; ?>"
                                                                ...
                                                                <?php if ($tipo === 'Varios' && $ins['cantidad'] > 1): ?>
                                                                    <div class="cantidad-input" style="display:none;">
                                                                        ...
                                                                        <label for="<?php echo $cid; ?>"
                                                                            class="small text-muted mb-0">Cant.</label>
                                                                        <input id="<?php echo $cid; ?>" type="number"
                                                                            class="form-control form-control-sm"
                                                                            name="cantidad_varios[<?php echo $ins['id_insumo']; ?>]"
                                                                            min="1" max="<?php echo (int) $ins['cantidad']; ?>"
                                                                            value="1" style="width:84px;">
                                                                    </div>
                                                                <?php endif; ?>
                                                                <button type="button"
                                                                    class="btn btn-sm btn-outline-primary btn-seleccionar"
                                                                    data-insumo-id="<?php echo $ins['id_insumo']; ?>"
                                                                    onclick="toggleSeleccionInsumo(<?php echo $ins['id_insumo']; ?>)"
                                                                    title="Seleccionar"
                                                                    aria-label="Seleccionar insumo <?php echo htmlspecialchars($displayName); ?>"><i
                                    ...
                                    <div class="d-flex justify-content-end gap-2 mt-4 mb-3 pe-3 pe-md-4">
                                        ...
                                        <button type="button" class="btn btn-primary"
                                            onclick="mostrarModalConfirmacion()"><i class="fas fa-eye me-1"></i>Revisar
                                            y Confirmar</button>
                                    </div>
                                </div>
                            </div>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title" id="modalConfirmacionLabel"><i
                            class="fas fa-check-circle me-2"></i>Confirmar Asignación</h5>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-success" onclick="confirmarAsignacion()"><i
                            class="fas fa-check me-1"></i>Confirmar</button>
                    <button type="button" class="btn btn-primary" onclick="confirmarEImprimir()"><i
                            class="fas fa-print me-1"></i>Confirmar e Imprimir</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        <?php if (isset($_GET['insumo_id']) && is_numeric($_GET['insumo_id'])): ?>
                // Atajo "Asignación Rápida": pre-seleccionar el insumo solamente.
                // No avanzamos aut omáticamente al Paso 2 para evitar d isparar la validación del Paso 1.
                $(document).ready(function () {
                    setTimeout(function () {
                        const btnSel = $('.btn-seleccionar[data-insumo-id="<?php echo (int) $_GET['insumo_id']; ?>"]');
                        if (btnSel.length > 0 && btnSel.hasClass('btn-outline-primary')) {
                            btnSel.click(); // Seleccionar el insumo visualmente
                        }
                    }, 300);
                });
        <?php endif; ?>
        ...
        function mostrarModalConfirmacion() {
            ...
            $('.hidden-insumo-input:not(:disabled)').each(function () {
                ...
                const tipo = $h.data('tipo');
                // Obtener ID físico y número de serie
                const idFisico = $fila.data('id-fisico') || '';
                const numSerie = $fila.data('numero-serie') || '';
                let infoExtra = [];
                if (idFisico) infoExtra.push('ID: ' + idFisico);
                if (numSerie) infoExtra.push('S/N: ' + numSerie);
                const infoExtraHtml = infoExtra.length ? `<div class="text-muted small">${infoExtra.join(' | ')}</div>` : '';
                let cant = '1';
                if (tipo === 'Varios') {
                    const input = $fila.find(`input[name="cantidad_varios[${$h.val()}]"]`);
                    cant = input.length ? input.val() : '1';
                }
                rows += `<tr><td><strong>${nombre}</strong>${infoExtraHtml}</td><td><span class=\"badge bg-info\">${tipo}</span></td><td><input type=\"number\" min=\"1\" class=\"form-control form-control-sm\" value=\"${cant}\" data-edit-id=\"${$h.val()}\" ${tipo === 'Varios' ? '' : 'disabled'}></td></tr>`;
            });
            rows += '</tbody></table>';
            $('#m_insumos').html(rows);
